...
impl Sorting {
	...
	pub fn sort_items(&self, items: &mut [TreeItem]) {
		...
		items.sort_by(|a, b| {
			...
			if self.reverse { res.reverse() } else { res }
		});
	...
	fn compare_items(&self, a: &TreeItem, b: &TreeItem) -> Ordering {
		...
		let clean_a = if self.ignore_leading_dot { label_a.strip_prefix('.').unwrap_or(label_a) } else { label_a };

		let clean_b = if self.ignore_leading_dot { label_b.strip_prefix('.').unwrap_or(label_b) } else { label_b };
