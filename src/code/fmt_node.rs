...
impl InternalNode {
	pub fn to_tree_item(&self, label: String) -> TreeItem {
		let children =
			self.children.iter().map(|(child_name, child_node)| child_node.to_tree_item(child_name.clone())).collect();
	...
	pub fn insert_path(
		...
		for (i, part) in parts.into_iter().enumerate() {
			...
			let node = current.children.entry(part.to_string()).or_insert_with(|| InternalNode {
	...
	pub fn build_root(res: &QueryResults) -> TreeItem {
		...
		for dir in &res.dirs {
			Self::insert_path(&mut root, &dir.path, true, false, 0, dir.matched_size, dir.real_size, dir.modified_at);
		}
		for file in &res.files {
			Self::insert_path(&mut root, &file.path, false, file.is_binary, file.size, 0, 0, file.modified_at);
		}

		let root_name = Path::new(&res.execution_dir).file_name().and_then(|s| s.to_str()).unwrap_or(".").to_string();
		...
		let children_items: Vec<TreeItem> =
			root.children.into_iter().map(|(child_name, child_node)| child_node.to_tree_item(child_name)).collect();
