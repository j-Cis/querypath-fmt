...
use crate::tree::{COL_NAME_SIZE_MIN, COL_PATH_SIZE_MIN, TREE_SYMBOLS, TreeItem};
...
impl<'a> FmtRender<'a> {
	pub fn render(&self, root_item: &TreeItem, max_prefix_len: usize) -> String {
		let eff_name_width = self.name_width.max(COL_NAME_SIZE_MIN); // <-- Minimum 15
		...
		let max_num = if total_numerated == 0 { 0 } else { self.numeration.start_from + total_numerated - 1 };

		let mut max_weight_inner_len = 0;
		let mut max_temporal_len = 0;
		self.collect_max_widths(root_item, &mut max_weight_inner_len, &mut max_temporal_len);
	...
	fn collect_max_widths(&self, item: &TreeItem, max_weight_inner_len: &mut usize, max_temporal_len: &mut usize) {
		match item {
			TreeItem::Spacer => {}
	...
	fn render_column_cell(
		...
		match col {
			...
			Column::Path => {
				let eff_path_width = self.path_width.max(COL_PATH_SIZE_MIN); // <-- Minimum 25
		...
		match item {
			TreeItem::Spacer => {
				for col in self.left_columns {
					out.push_str(&self.render_column_cell(
						*col,
						false,
						"",
						"",
			...
			TreeItem::Root { label, path, matched_size, real_size, modified_at, children } => {
				...
				let cont_prefix: String =
					line_prefix.chars().map(|c| if matches!(c, '│' | '├' | '┬' | '┌') { '│' } else { ' ' }).collect();
